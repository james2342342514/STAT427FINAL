...
threshold4 <- median(data_clean$scores_research, na.rm = TRUE)

data_clean$High_Research_Influence <- ifelse(data_clean$scores_research > threshold4, "High", "Low")
data_clean$High_Research_Influence <- as.factor(data_clean$High_Research_Influence)

predictors4 <- model.matrix(High_Research_Influence ~ . - 1 - scores_research, data = data_clean)
response4 <- data_clean$High_Research_Influence

set.seed(123)  
train_indices4 <- createDataPartition(response4, p = 0.8, list = TRUE)
train_data4 <- predictors4[train_indices4[[1]], ]
train_response4 <- response4[train_indices4[[1]]]
test_data4 <- predictors4[-train_indices4[[1]], ]
test_response4 <- response4[-train_indices4[[1]]]

logistic_model4 <- glm(High_Research_Influence ~ ., family = binomial(), data = data_clean)

test_predictions4 <- predict(logistic_model4, newdata = test_data, type = "response")
predicted_classes4 <- ifelse(test_predictions > 0.5, "High", "Low")

confusion_matrix4 <- table(Predicted = predicted_classes, Actual = test_response4)
...
library(pROC)
roc_result4 <- roc(test_response4, test_predictions4)
plot(roc_result4, main = "ROC Curve for High Research Influence")
auc_value4 <- auc(roc_result4)
